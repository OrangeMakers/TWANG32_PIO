...
// Wall Saver
#define WALL_TIMEOUT_UP 4000 // For how long should it be 90 degrees before it shuts down
#define WALL_TIMEOUT_DOWN 3000 // For how long should it be 90 degrees before it shuts down
...
// Debug
long debugEverySecondTimeout = 0;
...
long lastJoystickStageSwitch;

void debugEverySecond()
{
  if (millis() - 1000 > debugEverySecondTimeout)
  {
    debugEverySecondTimeout = millis();
    Serial.println("--- Debug");
    // Serial.print("Joystick Tilt: ");
    // Serial.println(joystickTilt);

    int angle_max = WALL_ANGLE - user_settings.joystick_deadzone;
    int angle_min = 0 - angle_max;
    // Serial.print("Angle min/max: ");
    ...
    if (joystickTilt < angle_min || joystickTilt > angle_max)
    {
      Serial.println("On Wall");
    }
    else
    {
      Serial.println("Not on wall");
    }

    int _joystickTilt = joystickTilt;
    if (_joystickTilt != prevAngle)
    {
      prevAngle = _joystickTilt;
      Serial.print("New angle: ");
      Serial.println(_joystickTilt);
    }
  }
}
...
void checkJoystickDirection(long mm)
{

  int angle_max = WALL_ANGLE - (user_settings.joystick_deadzone / 2);
  int angle_min = 0 - angle_max;
  ...
  {
  case UNKNOWN:
    if ((_joystickTilt > angle_min) & (_joystickTilt < angle_max))
    {
      joystickState = NORMAL;
      Serial.println("Joystick state to normal");
    }
    break;
  case NORMAL:
    if ((_joystickTilt > angle_max) & (mm - lastJoystickStageSwitch > 2000))
    {
      joystickState = UP;
      lastJoystickStageSwitch = mm;
      Serial.println("Joystick state to up");
    }
    else if ((_joystickTilt < angle_min) & (mm - lastJoystickStageSwitch > 2000))
    {
      joystickState = DOWN;
      lastJoystickStageSwitch = mm;
      Serial.println("Joystick state to down");
    }
    break;
  case UP:
    if ((_joystickTilt < angle_max) & (mm - lastJoystickStageSwitch > 2000))
    ...
    if (mm - lastJoystickStageSwitch > WALL_TIMEOUT_UP)
    {
    ...
    break;
  case DOWN:
    if ((_joystickTilt > angle_min) & (mm - lastJoystickStageSwitch > 1000))
    {
      joystickState = NORMAL;
      lastJoystickStageSwitch = mm;
      Serial.println("Joystick state to normal");
    }
    if (mm - lastJoystickStageSwitch > WALL_TIMEOUT_DOWN)
    {
      if (stage != SCREENSAVER)
        stage = SCREENSAVER;
    }
...
void loop()
{
  ...
  checkJoystickDirection(mm); // Check if hanging on the wall
  ...
  if (mm - previousMillis >= MIN_REDRAW_INTERVAL)
  {
    ...
    {
      lastInputTime = mm;
      if ((stage == SCREENSAVER) & (joystickState == NORMAL))
      {
        levelNumber = -1;
        stageStartTime = mm;
        stage = WIN;
